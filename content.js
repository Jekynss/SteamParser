...
function GemCosts(idss,gems) {
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            var res=parseInt(link.replace("p","").replace("у","").replace("б","").replace(".","").replace(",","."))-math;

            console.log(name+" : "+gems+" : "+link+" стоит сделать за:"+math);
            if(link!="0 pуб.")
            console.log(name+" : "+pers +"   ---"+ "выгода: "+res);
            else
            console.log(name+" : "+pers + "   выгода: ПРОВЕРИТЬ!!!!!!");
...
//{"name":"Slingshot people","price":"1200"},
function OtherSets() {
    $.ajax({
        type:'GET',
